    ...
    where TController : ICharacterController
{
    ...
    protected async UniTask<TController> CreateCharacter(Vector3 position, Transform parent, CharacterType type)
    {
        ...
        _poolable = await _characterPooler.Pull<IMyPoolable>(type, position, Quaternion.identity, parent);
        CharacterView characterView = _poolable.gameObject.GetComponent<CharacterView>();

        _poolable = await _characterUIPooler.Pull<IMyPoolable>(UIType.CharacterUI, position, Quaternion.Euler(90, 0, 0), parent);
